...
-- Rider with the most Succesfull deliveries

select top 5 r.rider_id,r.Rider_name,count(d.delivery_id) Number_of_deliveries from riders as r
inner join deliveries as d on r.rider_id = d.rider_id
...
select r.city, avg(datediff(minute,o.order_time,d.delivery_time)) as average_delivery_time
from restaurants as r
inner join orders as o on o.restaurant_id = r.restaurant_id
inner join deliveries as d on d.order_id = o.order_id
group by r.city
order by avg(datediff(minute,o.order_time,d.delivery_time))  ;
